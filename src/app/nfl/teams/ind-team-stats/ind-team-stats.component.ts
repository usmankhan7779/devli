...
import { TeamRankingsService } from '../team-rankings/team-rankings.service';
...
@Component({
  selector: 'app-ind-team-stats',
  templateUrl: './ind-team-stats.component.html',
  styleUrls: ['./ind-team-stats.component.scss']
})
export class IndTeamStatsComponent implements OnInit {
  ...
  onYearDdChange(season) {
    ...
    this.spinnerService.handleAPICall(this.teamService.getIndTeamStatsData(this.params.team_name, season.year))
      .subscribe(res => {
  ...
  private handleResponse(data, year?) {
    ...
    this.schemaService.addSchema([
        ...
        `https://www.lineups.com${this.data.nav.team_stats_route}`,
        'Dataset',
      ),
      {
        ...
        'url': `https://www.lineups.com${this.data.nav.team_stats_route}`,
    ...
    this.breadcrumbService.changeBreadcrumbs([
      {label: 'NFL', url: '/nfl'},
      {label: 'NFL Team Stats', url: '/nfl/team-stats'},
      {label: this.data.page_heading, url: this.data.nav.team_stats_route},
    ]);
  ...
  checkData() {
    try {
      ...
      this.hasData = !!(this.data.team_offensive_stats.length && this.data.team_defensive_stats.length &&
        this.data.special_team_stats.length && this.data.quarterback_stats.length && this.data.running_back_stats.length);
    } catch (e) {
      this.hasData = false;
    }
  }
